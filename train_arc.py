...
from model_arc import ARCModel
...
def train(model, data, Config, lr, eps, num_wav, len_frame):
    ...
    with tf.Session() as sess:
        ...
        for step in range(global_step.eval(), eps):
            ...
            src1_batch, _, _ = model.spec_to_batch(src1_mag)
            print("Source1 shape", src1_batch.shape)
            src2_batch, _, _ = model.spec_to_batch(src2_mag)
            print("Source2 shape:", src2_batch.shape)
            mixed_batch, _, freq = model.spec_to_batch(mixed_mag)
            print("Mixed Batch shape:", mixed_batch.shape)
...
if __name__ == '__main__':
    ...
    # Create model object
    model = ARCModel(sample_rate=args.sr, len_frame=args.lfr, seq_len=args.sql)
